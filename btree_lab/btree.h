...
class BTreeIndex {
 ...
 protected:
  ...
  ERROR_T      LookupOrUpdateInternal(const SIZE_T &Node,
				      ...
				      const KEY_T &key,
				      VALUE_T &val);
  ERROR_T      InsertByPosition(const SIZE_T &Node,
                                const BTreeOp op,
                                const KEY_T &key,
                                const VALUE_T &val);

  ERROR_T      BTreeInsertNoFull(const SIZE_T &Node,
				 const KEY_T &key,
				 const  VALUE_T &val);
  ERROR_T      BTreeSplitChild(const SIZE_T &Node,
			       const SIZE_T offset,
			       const VALUE_T &val); 
  ERROR_T      check_Tree(set<SIZE_T> visited, const SIZE_T &node) const;
